...
function Dashboard() {
  ...
  useEffect(() => {
    const calcNext = () => {
      ...
      if (nearestJob !== null) {
        ...
        setNextJob({ name: jobInfo.name, schedule: jobInfo.schedule, timeLeft: jobInfo.time.getTime() - now.getTime() });
      }
      setCurrentTime(now);
    };
    calcNext();
  ...
  const features = ['Auto Model Selection','Memory Persistence','Kanban Integration','Browser Automation','Skill System','Cron Scheduling'];
  ...
  return (
    <div className="flex min-h-screen bg-gradient-to-br from-gray-950 via-gray-900 to-black">
      ...
      {mobileMenuOpen && (
        <>
          <div onClick={() => setMobileMenuOpen(false)} className="fixed inset-0 bg-black/50 z-[999]" />
          <div className="fixed top-[72px] right-4 bg-gray-800 border border-gray-700 rounded-xl p-2 z-[1000] min-w-[200px] shadow-xl">
            <div className="px-3 py-2 text-gray-400 text-xs font-semibold uppercase">Publications</div>
            <a href="/videocue" className="flex items-center gap-3 px-3 py-3 text-white no-underline rounded-lg hover:bg-gray-700 transition-colors"><span className="text-xl">🎬</span><span className="text-sm">Video Board</span></a>
            <a href="/articles" className="flex items-center gap-3 px-3 py-3 text-white no-underline rounded-lg hover:bg-gray-700 transition-colors"><span className="text-xl">📰</span><span className="text-sm">Article Board</span></a>
            <a href="/ideas" className="flex items-center gap-3 px-3 py-3 text-white no-underline rounded-lg hover:bg-gray-700 transition-colors"><span className="text-xl">💡</span><span className="text-sm">Idea Board</span></a>
            <a href="/resourcelibrary" className="flex items-center gap-3 px-3 py-3 text-white no-underline rounded-lg hover:bg-gray-700 transition-colors"><span className="text-xl">📑</span><span className="text-sm">Resource Library</span></a>
            <a href="/wishlist" className="flex items-center gap-3 px-3 py-3 text-white no-underline rounded-lg hover:bg-gray-700 transition-colors"><span className="text-xl">🛒</span><span className="text-sm">Wishlist</span></a>
            <a href="/projects" className="flex items-center gap-3 px-3 py-3 text-white no-underline rounded-lg hover:bg-gray-700 transition-colors"><span className="text-xl">📂</span><span className="text-sm">Projects</span></a>
            <div className="h-px bg-gray-700 my-2" />
            <a href="/" className="flex items-center gap-3 px-3 py-3 text-white no-underline rounded-lg bg-purple-900/20 transition-colors"><span className="text-xl">🎛️</span><span className="text-sm">Command Center</span></a>
            <a href="/teamboard" className="flex items-center gap-3 px-3 py-3 text-white no-underline rounded-lg hover:bg-gray-700 transition-colors"><span className="text-xl">👥</span><span className="text-sm">Team Board</span></a>
            <a href="/openclaw" className="flex items-center gap-3 px-3 py-3 text-white no-underline rounded-lg hover:bg-gray-700 transition-colors"><span className="text-xl">🤖</span><span className="text-sm">OpenClaw Board</span></a>
          </div>
        </>
      )}
      ...
      <div className="max-w-7xl mx-auto relative">
        <div className={`flex flex-col md:flex-row md:items-center justify-between mb-6 gap-4 ${loaded ? 'animate-fadeInUp' : 'opacity-0'}`}>
          ...
          <div className="text-left md:text-right">
            ...
            <a href="/commands" className="inline-flex items-center gap-2 mt-2 px-3 py-1 bg-blue-600 hover:bg-blue-500 rounded-lg text-xs transition-colors">
              🎬 Command Center →
            </a>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 space-y-6">
            <div className={`bg-gray-900/50 rounded-xl p-4 md:p-6 border border-gray-700/50 ${loaded ? 'animate-slideInLeft delay-300' : 'opacity-0'}`}>
              <h2 className="text-base md:text-lg font-semibold mb-4 flex items-center gap-2"><span>🔑</span> AI API Keys</h2>
              <div className="space-y-2">
                {aiApiKeys.map((key, i) => (
                  <div key={i} className="flex items-center justify-between p-3 bg-gray-800/50 rounded-lg border border-gray-700/50 hover:border-cyan-500/50 hover:bg-gray-800/70 transition-all duration-300 hover:scale-[1.02] hover:shadow-lg hover:shadow-cyan-500/10">
                    <div className="flex items-center gap-3">
            ...
            <div className={`bg-gray-900/50 rounded-xl p-4 md:p-6 border border-gray-700/50 ${loaded ? 'animate-slideInLeft delay-400' : 'opacity-0'}`}>
              <h2 className="text-base md:text-lg font-semibold mb-4 flex items-center gap-2"><span>💰</span> API Spend (This Month)</h2>
            ...
            <div className={`bg-gray-900/50 rounded-xl p-4 md:p-6 border border-gray-700/50 ${loaded ? 'animate-slideInLeft delay-500' : 'opacity-0'}`}>
              <h2 className="text-base md:text-lg font-semibold mb-4 flex items-center gap-2"><span>🧠</span> AI Model Stack</h2>
              <div className="space-y-2">
                {models.map((model, i) => (
            ...
            <div className={`bg-gray-900/50 rounded-xl p-4 md:p-6 border border-gray-700/50 ${loaded ? 'animate-slideInLeft delay-600' : 'opacity-0'}`}>
              <h2 className="text-base md:text-lg font-semibold mb-4 flex items-center gap-2"><span>⏰</span> Automated Cron Jobs</h2>
          ...
          <div className="space-y-6">
            <div className={`bg-gray-900/50 rounded-xl p-4 md:p-6 border border-gray-700/50 ${loaded ? 'animate-slideInRight delay-300' : 'opacity-0'}`}>
              <h2 className="text-base md:text-lg font-semibold mb-4 flex items-center gap-2"><span>📋</span> Tasks</h2>
              <div className="space-y-4">
                {tasksByStatus.inProgress.length > 0 && (
                  <div>
                    ...
                    {tasksByStatus.inProgress.map((task, i) => <TaskItem key={i} task={task} />)}
                  </div>
                )}
                <div>
                  ...
                  <div className="space-y-1 max-h-40 overflow-y-auto">
                    {tasksByStatus.backlog.slice(0, 5).map((task, i) => <TaskItem key={i} task={task} />)}
                  </div>
                </div>
                <div>
                  ...
                  {tasksByStatus.done.slice(0, 3).map((task, i) => <TaskItem key={i} task={task} />)}
                </div>
              </div>
            </div>
            ...
            <div className={`bg-gray-900/50 rounded-xl p-4 md:p-6 border border-gray-700/50 ${loaded ? 'animate-slideInRight delay-400' : 'opacity-0'}`}>
              <h2 className="text-base md:text-lg font-semibold mb-4 flex items-center gap-2"><span>📡</span> Connected Channels</h2>
              <div className="space-y-2">
            ...
            <div className={`bg-gray-900/50 rounded-xl p-4 md:p-6 border border-gray-700/50 ${loaded ? 'animate-slideInRight delay-500' : 'opacity-0'}`}>
              <h2 className="text-base md:text-lg font-semibold mb-4 flex items-center gap-2"><span>🔌</span> 3rd Party Integrations</h2>
            ...
            <div className={`bg-gray-900/50 rounded-xl p-4 md:p-6 border border-gray-700/50 ${loaded ? 'animate-slideInRight delay-600' : 'opacity-0'}`}>
              <h2 className="text-base md:text-lg font-semibold mb-4 flex items-center gap-2"><span>⚡</span> Key Features</h2>
  ...
  return (
    <div className="bg-gray-800/50 rounded-xl p-3 md:p-4 border border-gray-700/50 hover:border-cyan-500/50 transition-all duration-300 hover:scale-105 animate-pulseGlow">
      <div className="flex items-center gap-2 md:gap-3">
        ...
        <div>
          <div className={`text-xl md:text-2xl font-bold text-cyan-400 tabular-nums ${!isText ? 'animate-numberFlash' : ''}`}>{value}</div>
...
function StatusBadge({ status }) {
  ...
  return <span className={`px-2 py-1 rounded text-xs border ${styles[status] || styles.available}`}>{status}</span>;
}

function TaskItem({ task }) {
  const priorityColors = { high: 'text-red-400', med: 'text-yellow-400', low: 'text-gray-400' };
  return (
    <div className="flex items-center gap-2 p-2 bg-gray-800/30 rounded text-sm hover:bg-gray-800/50 transition-all duration-300">
      ...
      {task.priority && <span className={`text-xs ${priorityColors[task.priority]}`}>[{task.priority}]</span>}
    </div>
  );
}
  ...
  return (
    <div className="p-3 bg-gray-800/50 rounded-lg border border-gray-700/50 hover:border-cyan-500/30 transition-all duration-300">
      ...
      <div className="w-full bg-gray-700 rounded-full h-2 overflow-hidden">
        <div className="bg-gradient-to-r from-cyan-500 to-blue-500 h-2 rounded-full transition-all duration-1000 ease-out" style={{ width: `${width}%` }} />
      </div>
    </div>
  );
}
